...
function _M0TP211localreview4imap7Decoder(param0, param1, param2, param3, param4) {
  this.buffer = param0;
  this.head = param1;
...
const $bytes_literal$1 = new Uint8Array([13,10]);
...
const _M0FPB11find__indexN6constrS10508 = 0;
const _M0FPB11find__indexN6constrS10509 = 0;
...
function _M0MPC15bytes5Bytes20unsafe__range__equal(self, self_off, other, other_off, len) {
  let _tmp = 0;
  while (true) {
    const i = _tmp;
    if (i < len) {
      if (_M0IPC14byte4BytePB2Eq5equal(self[self_off + i | 0], other[other_off + i | 0])) {
      } else {
        return false;
      }
      _tmp = i + 1 | 0;
...
function _M0IPC15bytes9BytesViewPB2Eq5equal(self, other) {
  const len = self.end - self.start | 0;
  return len === (other.end - other.start | 0) ? _M0MPC15bytes5Bytes20unsafe__range__equal(self.buf, self.start, other.buf, other.start, len) : false;
}
...
function _M0FPB22find__byte__from__view(target, start, end, byte) {
  let _tmp = start;
  while (true) {
    const pos = _tmp;
    if (pos < end) {
      if (_M0IPC14byte4BytePB2Eq5equal(target.buf[target.start + pos | 0], byte)) {
        return pos;
      }
      _tmp = pos + 1 | 0;
      continue;
...
function _M0FPB13find__cutover(i) {
  if (16 === 0) {
    $panic();
  }
  return 4 + (i / 16 | 0) | 0;
}
function _M0FPB23find__rabin__karp__fromN4hashS1958(bytes, start, length) {
  let _tmp = 0;
  let _tmp$2 = 0;
  while (true) {
    const i = _tmp;
    ...
    if (i < length) {
      _tmp = i + 1 | 0;
      _tmp$2 = ((Math.imul(hash, 16777619) | 0) >>> 0) + (_M0MPC14byte4Byte8to__uint(bytes.buf[bytes.start + (start + i | 0) | 0]) >>> 0) | 0;
      continue;
    } else {
      return hash;
    }
...
function _M0FPB23find__rabin__karp__from(target, pattern, start) {
  const pattern_len = pattern.end - pattern.start | 0;
  const target_len = target.end - target.start | 0;
  if ((start + pattern_len | 0) > target_len) {
    return undefined;
  }
  const target_start = _M0MPC15bytes9BytesView13start__offset(target);
  const pattern_start = _M0MPC15bytes9BytesView13start__offset(pattern);
  const pattern_hash = _M0FPB23find__rabin__karp__fromN4hashS1958(pattern, 0, pattern_len);
  let pow;
  let _tmp = 1;
  let _tmp$2 = 16777619;
  let _tmp$3 = pattern_len;
  while (true) {
    const pow$2 = _tmp;
    const square = _tmp$2;
    const exp = _tmp$3;
    if (exp > 0) {
      ...
      if ((exp % 2 | 0) === 1) {
        next_pow = Math.imul(pow$2, square) | 0;
      } else {
        next_pow = pow$2;
      }
      _tmp = next_pow;
      _tmp$2 = Math.imul(square, square) | 0;
      if (2 === 0) {
        $panic();
      }
      _tmp$3 = exp / 2 | 0;
  ...
  const hash = _M0FPB23find__rabin__karp__fromN4hashS1958(target, start, pattern_len);
  if (hash === pattern_hash && _M0MPC15bytes5Bytes20unsafe__range__equal(_M0MPC15bytes9BytesView4data(target), target_start + start | 0, _M0MPC15bytes9BytesView4data(pattern), pattern_start, pattern_len)) {
    return start;
  }
  let _tmp$4 = start + pattern_len | 0;
  let _tmp$5 = hash;
  while (true) {
    const index = _tmp$4;
    const hash$2 = _tmp$5;
    if (index < target_len) {
      const hash$3 = ((Math.imul(hash$2, 16777619) | 0) >>> 0) + (_M0MPC14byte4Byte8to__uint(target.buf[target.start + index | 0]) >>> 0) | 0;
      const hash$4 = (hash$3 >>> 0) - ((Math.imul(pow, _M0MPC14byte4Byte8to__uint(target.buf[target.start + (index - pattern_len | 0) | 0])) | 0) >>> 0) | 0;
      const candidate = (index - pattern_len | 0) + 1 | 0;
      if (hash$4 === pattern_hash && _M0MPC15bytes5Bytes20unsafe__range__equal(_M0MPC15bytes9BytesView4data(target), target_start + candidate | 0, _M0MPC15bytes9BytesView4data(pattern), pattern_start, pattern_len)) {
        return candidate;
      }
      _tmp$4 = index + 1 | 0;
      _tmp$5 = hash$4;
...
function _M0FPB29find__long__by__byte__scanner(target, pattern) {
  const target_len = target.end - target.start | 0;
  const pattern_len = pattern.end - pattern.start | 0;
  const last = target_len - pattern_len | 0;
  const target_start = _M0MPC15bytes9BytesView13start__offset(target);
  const pattern_start = _M0MPC15bytes9BytesView13start__offset(pattern);
  const first_byte = pattern.buf[pattern.start];
  let _tmp = 0;
  let _tmp$2 = 0;
  while (true) {
    const pos = _tmp;
    const failures = _tmp$2;
    if (pos <= last) {
      const found = _M0FPB22find__byte__from__view(target, pos, target_len, first_byte);
      if (found < 0 || found > last) {
        return undefined;
      }
      if (_M0MPC15bytes5Bytes20unsafe__range__equal(_M0MPC15bytes9BytesView4data(target), (target_start + found | 0) + 1 | 0, _M0MPC15bytes9BytesView4data(pattern), pattern_start + 1 | 0, pattern_len - 1 | 0)) {
        return found;
      }
      const failures$2 = failures + 1 | 0;
      if (failures$2 > 64 || failures$2 > _M0FPB13find__cutover(found)) {
        return _M0FPB23find__rabin__karp__from(target, pattern, found + 1 | 0);
      }
      _tmp = found + 1 | 0;
      _tmp$2 = failures$2;
      continue;
...
function _M0FPB11find__short(target, pattern) {
  const target_len = target.end - target.start | 0;
  const pattern_len = pattern.end - pattern.start | 0;
  const target_start = _M0MPC15bytes9BytesView13start__offset(target);
  const pattern_start = _M0MPC15bytes9BytesView13start__offset(pattern);
  const last_offset = pattern_len - 1 | 0;
  const last_byte = pattern.buf[pattern.start + last_offset | 0];
  let _tmp = last_offset;
  while (true) {
    const anchor_pos = _tmp;
    if (anchor_pos < target_len) {
      const found = _M0FPB22find__byte__from__view(target, anchor_pos, target_len, last_byte);
      if (found < 0) {
        return undefined;
      }
      const candidate = found - last_offset | 0;
      if (_M0MPC15bytes5Bytes20unsafe__range__equal(_M0MPC15bytes9BytesView4data(target), target_start + candidate | 0, _M0MPC15bytes9BytesView4data(pattern), pattern_start, last_offset)) {
        return candidate;
      }
      _tmp = found + 1 | 0;
      continue;
...
function _M0FPB11find__index(target, pattern) {
  const target_len = target.end - target.start | 0;
  const pattern_len = pattern.end - pattern.start | 0;
  if (pattern_len === 0) {
    return _M0FPB11find__indexN6constrS10508;
  }
  if (pattern_len > target_len) {
    return undefined;
  }
  if (pattern_len === 1) {
    const found = _M0FPB22find__byte__from__view(target, 0, target_len, pattern.buf[pattern.start]);
    return found < 0 ? undefined : found;
  } else {
    return pattern_len === target_len ? (_M0IPC15bytes9BytesViewPB2Eq5equal(target, pattern) ? _M0FPB11find__indexN6constrS10509 : undefined) : pattern_len <= 32 ? _M0FPB11find__short(target, pattern) : _M0FPB29find__long__by__byte__scanner(target, pattern);
  }
}
function _M0MPC15bytes9BytesView4find(target, pattern) {
  return _M0FPB11find__index(target, pattern);
}
function _M0MPC15bytes5Bytes4find(target, pattern) {
  return _M0MPC15bytes9BytesView4find(new _M0TPC15bytes9BytesView(target, 0, target.length), pattern);
}
...
function _M0MPC15array5Array28unsafe__truncate__to__lengthGzE(self, new_len) {
...
function _M0MPC15array5Array4copyGzE(self) {
...
function _M0MPC15array5Array5clearGzE(self) {
  _M0MPC15array5Array28unsafe__truncate__to__lengthGzE(self, 0);
}
...
function _M0MPC15array5Array10push__iterGsE(self, iter) {
  ...
  while (true) {
    ...
    if (_bind$2 === undefined) {
      ...
      const _Some = _bind$2;
      const _x = _Some;
      _M0MPC15array5Array4pushGzE(self, _x);
      continue;
    }
  }
}
...
function _M0MPC15debug4Repr6record(fields) {
  ...
  while (true) {
    ...
    if (_bind === undefined) {
      ...
      const _name = _x._0;
      const _value = _x._1;
      _M0MPC15array5Array4pushGzE(_acc, new _M0DTPC15debug4Repr11RecordField(_name, _value));
      continue;
    }
  }
...
function _M0FPC15debug15surround__lines(start, finish, lines) {
  if (lines.length === 0) {
    ...
    if (lines.length === 1) {
      ...
      const _self = [];
      _M0MPC15array5Array4pushGzE(_self, `${start}${_first}`);
      _M0MPC15array5Array10push__iterGsE(_self, _M0MPC15array9ArrayView4iterGsE(_x));
      _M0MPC15array5Array4pushGzE(_self, `${_last}${finish}`);
      return _self;
    }
  }
}
      ...
      if (_first === "[" && _last === "]" || _first === "{" && _last === "}") {
        ...
        while (true) {
          ...
          if (_ < _bind) {
            ...
            if (_M0IP016_24default__implPB2Eq10not__equalGRPC16string10StringViewE(t, new _M0TPC16string10StringView(_M0FPC15debug14compact__linesN7_2abindS1134, 0, _M0FPC15debug14compact__linesN7_2abindS1134.length))) {
              _M0MPC15array5Array4pushGzE(parts, t);
            }
            _tmp = _ + 1 | 0;
            continue;
          } else {
        ...
        if (_M0MPC16string6String11has__suffix(_first, new _M0TPC16string10StringView(_M0FPC15debug14compact__linesN7_2abindS1148, 0, _M0FPC15debug14compact__linesN7_2abindS1148.length)) && _last === ")") {
          ...
          while (true) {
            ...
            if (_ < _bind) {
              const m = lines[1 + _ | 0];
              _M0MPC15array5Array4pushGzE(parts, _M0MPC16string6String4trim(m, undefined));
              _tmp = _ + 1 | 0;
              continue;
            } else {
              break;
          ...
          _M0MPC15array5Array4pushGzE(parts, _M0MPC16string6String4trim(_last, undefined));
  ...
  if (contents.length === 0) {
    ...
    if (contents.length === 1) {
      ...
      if (_item.length > 1) {
        ...
        const _self = [];
        _M0MPC15array5Array4pushGzE(_self, open);
        _M0MPC15array5Array10push__iterGsE(_self, _M0MPC15array5Array4iterGsE(lines));
        _M0MPC15array5Array4pushGzE(_self, close);
        return _self;
      } else {
        ...
        if (_ < _bind) {
          ...
          if (item_lines.length === 0) {
            ...
            if (item_lines.length === 1) {
              ...
              _M0MPC15array5Array4pushGzE(out, `${_M0MPC16string6String6repeat(" ", indent_by)}${_x},`);
            } else {
              const _first = item_lines[0];
              const _last = item_lines[item_lines.length - 1 | 0];
              const _x_end = item_lines.length - 1 | 0;
              _M0MPC15array5Array4pushGzE(out, `${_M0MPC16string6String6repeat(" ", indent_by)}${_first}`);
              ...
              while (true) {
                ...
                if (_$2 < _bind$2) {
                  ...
                  _M0MPC15array5Array4pushGzE(out, `${_M0MPC16string6String6repeat(" ", indent_by)}${mid}`);
                  _tmp$2 = _$2 + 1 | 0;
                  continue;
                } else {
                  break;
              ...
              _M0MPC15array5Array4pushGzE(out, `${_M0MPC16string6String6repeat(" ", indent_by)}${_last},`);
            }
          }
          _tmp = _ + 1 | 0;
          continue;
      ...
      _M0MPC15array5Array4pushGzE(out, close);
      return out;
    }
  }
}
          ...
          if (contents.length === 1) {
            ...
            if (item_lines.length === 0) {
              ...
              if (item_lines.length === 1) {
                ...
                const _self = [];
                _M0MPC15array5Array4pushGzE(_self, begin);
                ...
                _M0MPC15array5Array4pushGzE(_self, `${_M0MPC16string6String6repeat(" ", 2)}${_last_line},`);
                _M0MPC15array5Array4pushGzE(_self, end);
                return _self;
              }
            }
          } else {
            ...
            while (true) {
              ...
              if (_ < _bind) {
                ...
                if (item_lines.length === 0) {
                  ...
                  if (item_lines.length === 1) {
                    const _x = item_lines[0];
                    _M0MPC15array5Array4pushGzE(out, `${_M0MPC16string6String6repeat(" ", 2)}${_x},`);
                  } else {
                    const _first = item_lines[0];
                    const _last = item_lines[item_lines.length - 1 | 0];
                    const _x_end = item_lines.length - 1 | 0;
                    _M0MPC15array5Array4pushGzE(out, `${_M0MPC16string6String6repeat(" ", 2)}${_first}`);
                    const _bind$2 = _x_end - 1 | 0;
                    let _tmp$2 = 0;
                    while (true) {
                      const _$2 = _tmp$2;
                      if (_$2 < _bind$2) {
                        const mid = item_lines[1 + _$2 | 0];
                        _M0MPC15array5Array4pushGzE(out, `${_M0MPC16string6String6repeat(" ", 2)}${mid}`);
                        _tmp$2 = _$2 + 1 | 0;
                        continue;
                      } else {
                        break;
                    ...
                    _M0MPC15array5Array4pushGzE(out, `${_M0MPC16string6String6repeat(" ", 2)}${_last},`);
                  }
                }
                _tmp = _ + 1 | 0;
                continue;
            ...
            _M0MPC15array5Array4pushGzE(out, end);
            return out;
          }
        }
      }
      ...
      if (children.length === 2) {
        ...
        if (_bind.length === 0) {
          ...
          if (_bind.length === 1) {
            ...
            const _self = [];
            _M0MPC15array5Array4pushGzE(_self, head);
      ...
      if (children.length === 1) {
        ...
        if (_bind.length === 0) {
          ...
          if (_bind.length === 1) {
            ...
            const _self = [];
            _M0MPC15array5Array4pushGzE(_self, `${_name$3}=${_first}`);
            _M0MPC15array5Array10push__iterGsE(_self, _M0MPC15array9ArrayView4iterGsE(_x$6));
            return _M0FPC15debug10no__parens(_M0FPC15debug15content__parens(_tmp$2, _self));
          }
        }
      ...
      if (children.length === 1) {
        ...
        if (_bind.length === 0) {
          ...
          if (_bind.length === 1) {
            ...
            const _self = [];
            _M0MPC15array5Array4pushGzE(_self, `${label}: ${_first}`);
            _M0MPC15array5Array10push__iterGsE(_self, _M0MPC15array9ArrayView4iterGsE(_x$6));
            return _M0FPC15debug10no__parens(_M0FPC15debug15content__parens(_tmp$2, _self));
          }
        }
...
function _M0MP211localreview4imap7Decoder3new() {
  return new _M0TP211localreview4imap7Decoder($bytes_literal$0, "", [], -1, false);
}
function _M0FP211localreview4imap4text(b) {
  let _try_err;
  _L: {
    const _bind = _M0FPC28encoding4utf814decode_2einner(new _M0TPC15bytes9BytesView(b, 0, b.length), false);
    let _tmp;
    if (_bind.$tag === 1) {
      const _ok = _bind;
      _tmp = _ok._0;
...
function _M0FP211localreview4imap12suffix__size(line) {
  ...
  const cs = _M0MPC16string6String9to__array(line);
  const start = new _M0TPB8MutLocalGiE(cs.length - 2 | 0);
  while (true) {
    if (start.val >= 0 && _M0MPC15array5Array2atGcE(cs, start.val) !== 123) {
  ...
  if (start.val < 0) {
    return new _M0DTPC16result6ResultGiRP211localreview4imap9ImapErrorE3Err(new _M0DTPC15error5Error40localreview_2fimap_2eImapError_2eInvalid("literal marker"));
  }
  const size = _M0MPC16string6String11from__array(_M0MPC15array5Array12view_2einnerGcE(cs, start.val + 1 | 0, cs.length - 1 | 0));
  if (_M0MPC16string6String9is__empty(size) || !_M0MPB4Iter3allGcE(_M0MPC15array5Array4iterGcE(_M0MPC16string6String9to__array(size)), (c) => c >= 48 && c <= 57)) {
  ...
  _L: {
    _L$2: {
      const _bind$2 = _M0FPC28internal7strconv18parse__int_2einner(new _M0TPC16string10StringView(size, 0, size.length), 0);
      if (_bind$2.$tag === 1) {
        const _ok = _bind$2;
        n = _ok._0;
      } else {
...
function _M0MP211localreview4imap7Decoder7consume(self, bytes) {
  if ((self.buffer.length + bytes.length | 0) > 2097152) {
    return new _M0DTPC16result6ResultGRPB5ArrayGRP211localreview4imap8ResponseERP211localreview4imap9ImapErrorE3Err(new _M0DTPC15error5Error40localreview_2fimap_2eImapError_2eInvalid("buffer limit"));
  }
  const held = new _M0TPB8MutLocalGiE(0);
  const _bind = self.literals;
  ...
  if (((held.val + self.buffer.length | 0) + bytes.length | 0) > 2097152) {
    return new _M0DTPC16result6ResultGRPB5ArrayGRP211localreview4imap8ResponseERP211localreview4imap9ImapErrorE3Err(new _M0DTPC15error5Error40localreview_2fimap_2eImapError_2eInvalid("aggregate literal buffer limit"));
  }
  self.buffer = _M0IPC15bytes5BytesPB3Add3add(self.buffer, bytes);
  const out = [];
  while (true) {
    if (self.expected >= 0) {
      ...
      _M0MPC15array5Array4pushGzE(self.literals, _M0MPC15bytes5Bytes11from__array(_M0MPC15array5Array12view_2einnerGyE(_M0MPC15bytes5Bytes9to__array(self.buffer), 0, self.expected)));
      self.buffer = _M0MPC15bytes5Bytes11from__array(_M0MPC15array5Array12view_2einnerGyE(_M0MPC15bytes5Bytes9to__array(self.buffer), self.expected, undefined));
      self.expected = -1;
    }
    let end;
    const _tmp$2 = self.buffer;
    const _bind$3 = $bytes_literal$1;
    const _bind$4 = _M0MPC15bytes5Bytes4find(_tmp$2, new _M0TPC15bytes9BytesView(_bind$3, 0, _bind$3.length));
    ...
    const _bind$5 = _M0FP211localreview4imap4text(_M0MPC15bytes5Bytes11from__array(_M0MPC15array5Array12view_2einnerGyE(_M0MPC15bytes5Bytes9to__array(self.buffer), 0, end)));
    let line;
    if (_bind$5.$tag === 1) {
      const _ok = _bind$5;
      line = _ok._0;
    ...
    let _tmp$3;
    const _bind$6 = "\n";
    if (_M0MPC16string6String8contains(line, new _M0TPC16string10StringView(_bind$6, 0, _bind$6.length))) {
      _tmp$3 = true;
    } else {
      const _bind$7 = "\r";
      _tmp$3 = _M0MPC16string6String8contains(line, new _M0TPC16string10StringView(_bind$7, 0, _bind$7.length));
    }
    if (_tmp$3) {
      return new _M0DTPC16result6ResultGRPB5ArrayGRP211localreview4imap8ResponseERP211localreview4imap9ImapErrorE3Err(new _M0DTPC15error5Error40localreview_2fimap_2eImapError_2eInvalid("bare newline"));
    }
    self.buffer = _M0MPC15bytes5Bytes11from__array(_M0MPC15array5Array12view_2einnerGyE(_M0MPC15bytes5Bytes9to__array(self.buffer), end + 2 | 0, undefined));
    self.head = `${self.head}${line}`;
    if (self.head.length > 65536 || self.literals.length > 64) {
      return new _M0DTPC16result6ResultGRPB5ArrayGRP211localreview4imap8ResponseERP211localreview4imap9ImapErrorE3Err(new _M0DTPC15error5Error40localreview_2fimap_2eImapError_2eInvalid("response limit"));
    }
    const _bind$7 = _M0FP211localreview4imap12suffix__size(line);
    let n;
    if (_bind$7.$tag === 1) {
      const _ok = _bind$7;
      n = _ok._0;
    ...
    _M0MPC15array5Array4pushGzE(out, new _M0TP211localreview4imap8Response(self.head, _M0MPC15array5Array4copyGzE(self.literals)));
    self.head = "";
    _M0MPC15array5Array5clearGzE(self.literals);
    continue;
  }
...
function _M0MP211localreview4imap7Decoder4feed(self, bytes) {
  ...
  let _try_err;
  _L: {
    const _bind = _M0MP211localreview4imap7Decoder7consume(self, bytes);
    let _tmp;
    if (_bind.$tag === 1) {
      const _ok = _bind;
      _tmp = _ok._0;
    ...
    return new _M0DTPC16result6ResultGRPB5ArrayGRP211localreview4imap8ResponseERP211localreview4imap9ImapErrorE2Ok(_tmp);
  }
  let s;
  _L$2: {
    const _Invalid = _try_err;
    const _s = _Invalid._0;
    s = _s;
    break _L$2;
  }
  self.failed = true;
  return new _M0DTPC16result6ResultGRPB5ArrayGRP211localreview4imap8ResponseERP211localreview4imap9ImapErrorE3Err(new _M0DTPC15error5Error40localreview_2fimap_2eImapError_2eInvalid(s));
}
function _M0MP211localreview4imap7Decoder6finish(self) {
  if (self.failed || (self.expected >= 0 || (!_M0MPC15bytes5Bytes9is__empty(self.buffer) || !_M0MPC16string6String9is__empty(self.head)))) {
    self.failed = true;
...
function _M0FP411localreview4imap3cmd3web3run(input) {
  ...
  _L: {
    ...
    const _bind$2 = "\r\n";
    const _bind$3 = _M0MPC16string6String12replace__all(input, _tmp, new _M0TPC16string10StringView(_bind$2, 0, _bind$2.length));
    const _bind$4 = _M0MP211localreview4imap7Decoder4feed(d, _M0FPC28encoding4utf814encode_2einner(new _M0TPC16string10StringView(_bind$3, 0, _bind$3.length), false));
    let out;
    if (_bind$4.$tag === 1) {
      const _ok = _bind$4;
      out = _ok._0;
    ...
    const _bind$5 = _M0MP211localreview4imap7Decoder6finish(d);
    if (_bind$5.$tag === 1) {
      const _ok = _bind$5;
      _ok._0;
    } else {
...
export { _M0FP411localreview4imap3cmd3web3run as run }
